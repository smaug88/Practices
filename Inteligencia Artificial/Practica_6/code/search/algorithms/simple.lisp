...
(defun general-search (problem queuing-fn)
  ...
  (let ((nodes (make-initial-queue problem queuing-fn))             ; Inicializamos la variable 'nodes' con el problema actual definido
	node
        total)                                                       ; Crea la variable 'node'
    (setq total 0)
    (loop (if (empty-queue? nodes) (RETURN nil))                    ; Hace un bucle que finalizará cuando no queden nodos por comprobar
	  (setq node (remove-front nodes))                          ; Guarda en la variable 'node' el primer nodo de 'nodes'
          (setq total (+ total 1))
	  (if (goal-test problem (node-state node)) (RETURN node))  ; Comprueba si se da el caso final con el nodo actual, devuelve el nodo
	  (funcall queuing-fn nodes (expand node problem)))
    (print total)
    node))       ; en caso contrario, carga en nodes el siguiente nodo a comprobar.
