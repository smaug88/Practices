...
(defstructure (minimax5-game-agent
                (:include game-agent
                    (algorithm #'(lambda (state game)
                                    (minimax-cutoff-decision state game #'ttt-eval 5)
                                )
                    )
                )
            )
 "A game-playing agent that uses ttt-eval to do a minimax search with a max deep = 5"
)

(defstructure (minimax10-game-agent
                (:include game-agent
                    (algorithm #'(lambda (state game)
                                    (minimax-cutoff-decision state game #'ttt-eval 10)
                                )
                    )
                )
            )
 "A game-playing agent that uses ttt-eval to do a minimax search with a max deep = 10"
)
