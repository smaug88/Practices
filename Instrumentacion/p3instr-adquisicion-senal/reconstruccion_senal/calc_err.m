...
for count = 1:length(samples)
    ...
    [abserr maxerr meanerr relerr] = signal_error(A, w, p, samples(count).amplitude, samples(count).angfreq, samples(count).phase, 0:0.01:8*pi);
    err_struc         = struct;
    err_struc.abserr  = abserr;
    err_struc.maxerr  = maxerr;
